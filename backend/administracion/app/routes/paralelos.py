...
def crear_paralelo():
    ...
    if not data or any(not data.get(campo) for campo in campos):
        return jsonify({"error": "Nombre, carrera_id, asignatura_id, docente_id y periodo_id son obligatorios"}), 400

    db = SessionLocal()
    try:
        carrera = db.query(Carrera).filter(Carrera.id == data.get("carrera_id"), Carrera.estado == True).first()
        asignatura = db.query(Asignatura).filter(Asignatura.id == data.get("asignatura_id"), Asignatura.estado == True).first()
        docente = db.query(Docente).filter(Docente.id == data.get("docente_id"), Docente.estado == True).first()
        periodo = db.query(PeriodoAcademico).filter(PeriodoAcademico.id == data.get("periodo_id"), PeriodoAcademico.estado == True).first()
        ...
        return jsonify({"mensaje": "Paralelo creado correctamente"}), 201
...
def actualizar_paralelo(id):
    ...
    try:
        ...
        if data.get("carrera_id"):
            ...
            if not carrera:
                return jsonify({"error": "La carrera indicada no existe"}), 404
            paralelo.carrera_id = data.get("carrera_id")

        if data.get("asignatura_id"):
            asignatura = db.query(Asignatura).filter(Asignatura.id == data.get("asignatura_id"), Asignatura.estado == True).first()
            if not asignatura:
                return jsonify({"error": "La asignatura indicada no existe"}), 404
            paralelo.asignatura_id = data.get("asignatura_id")

        if data.get("docente_id"):
            docente = db.query(Docente).filter(Docente.id == data.get("docente_id"), Docente.estado == True).first()
            if not docente:
                return jsonify({"error": "El docente indicado no existe"}), 404
            paralelo.docente_id = data.get("docente_id")

        if data.get("periodo_id"):
            periodo = db.query(PeriodoAcademico).filter(PeriodoAcademico.id == data.get("periodo_id"), PeriodoAcademico.estado == True).first()
            if not periodo:
                return jsonify({"error": "El periodo académico indicado no existe"}), 404
            paralelo.periodo_id = data.get("periodo_id")
...
def eliminar_paralelo(id):
    ...
    try:
        ...
        registrar_auditoria(
            ...
            "ELIMINAR",
            "PARALELOS",
            f"Se eliminó lógicamente el paralelo con ID {id}"
        )

        return jsonify({"mensaje": "Paralelo eliminado correctamente"}), 200
